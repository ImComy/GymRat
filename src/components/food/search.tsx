...
const SearchFood: React.FC<SearchFoodProps> = ({ onSearch }) => {
  ...
  return (
    <div className='flex gap-5 mt-5 items-center md:border lg:border border-0 rounded-full flex-col-reverse lg:flex-row md:flex-row sm:flex-col-reverse border-gray-200 sm:mb-3'>
      <div className="cursor-pointer coupons-select-container relative w-full md:w-[120px] md:border-0 border border-solid border-gray-200 rounded-full lg:ml-2" ref={dropdownRef}>
        <div
          className={`coupons-select ${isOpen ? 'open' : ''} flex items-center justify-between w-full py-2 pl-3 pr-10 text-sm text-gray-700 rounded-md transition-all duration-300`}
          onClick={handleClick}
        >
          <span className="nf nf-fa-filter text-base text-gray-600 mr-2"></span>
          <span className="mr-2">{selectedOption}</span>
          <span className={`fas fa-angle-down text-base text-gray-600 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`} />
        </div>
        <div className={`dropdown-menu ${isOpen ? 'open' : ''} absolute z-10 w-full bg-white rounded-md shadow-md`}>
      ...
      <div className="flex justify-between w-full md:border-0 border border-solid border-gray-200 rounded-full">
